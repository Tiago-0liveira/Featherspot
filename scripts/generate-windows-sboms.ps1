...
$generatedName = "lspotify-$ReleaseTag.cdx.json"
cargo cyclonedx --format json --target x86_64-pc-windows-msvc --override-filename "lspotify-$ReleaseTag.cdx"
if ($LASTEXITCODE -ne 0) { throw 'Could not build the SBOMs.' }

New-Item -ItemType Directory -Force $OutputDir | Out-Null
foreach ($entry in @(
  @{ Package = 'lspotify-cli'; Suffix = 'cli' },
  @{ Package = 'lspotify-player-host'; Suffix = 'player-host' }
)) {
  ...
  $destination = Join-Path $OutputDir "lspotify-$ReleaseTag-$($entry.Suffix).cdx.json"

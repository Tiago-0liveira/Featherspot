...
use lspotify_core::{LocalPlayerCommand, LocalPlayerEvent};
...
impl BackgroundLocalPlayer {
    ...
    pub fn start() -> Self {
        ...
        {
            let executable =
                std::env::var_os("LSPOTIFY_PLAYER_HOST").map(PathBuf::from).or_else(|| {
                    std::env::current_exe().ok().and_then(|path| {
                        let directory = path.parent()?;
                        let candidate1 = directory.join("lspotify-player-host.exe");
                        if candidate1.is_file() {
                            return Some(candidate1);
                        }
                        let candidate2 = directory.join("helpers").join("lspotify-player-host.exe");

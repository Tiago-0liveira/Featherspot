...
use lspotify_core::{AppError, CacheRepository, Cached, ErrorKind, Result};
...
impl CacheRepository for SqliteCache {
    fn read(&self, key: String) -> lspotify_core::BoxFuture<'_, Result<Option<Cached<Vec<u8>>>>> {
    ...
    fn write(
        ...
        key: String,
        value: Cached<Vec<u8>>,
    ) -> lspotify_core::BoxFuture<'_, Result<()>> {
        Box::pin(async move { self.write_sync(&key, &value) })
    }

    fn invalidate(&self, prefix: String) -> lspotify_core::BoxFuture<'_, Result<usize>> {
        Box::pin(async move { self.invalidate_sync(&prefix) })
    }

    fn size_bytes(&self) -> lspotify_core::BoxFuture<'_, Result<u64>> {
        Box::pin(async move { self.size_sync() })
    }

    fn trim_to(&self, maximum_bytes: u64) -> lspotify_core::BoxFuture<'_, Result<u64>> {
        Box::pin(async move { self.trim_sync(maximum_bytes) })
    }

    fn clear(&self) -> lspotify_core::BoxFuture<'_, Result<()>> {
        Box::pin(async move {
            self.connection
                .lock()
                .map_err(|_| lock_error())?
...
fn apply_migrations(connection: &mut Connection) -> Result<()> {
    ...
    if version > 1 {
        return Err(AppError::new(
            ErrorKind::Storage,
            "cache database was created by a newer lspotify version",
        ));
    }
    Ok(())
}

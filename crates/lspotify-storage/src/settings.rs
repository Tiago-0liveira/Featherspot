...
use lspotify_core::{AppError, ErrorKind, Result, SETTINGS_SCHEMA_VERSION, SettingsState};
    ...
    pub fn load(&self) -> Result<SettingsState> {
        ...
        if settings.schema_version > SETTINGS_SCHEMA_VERSION {
            return Err(AppError::new(
                ErrorKind::Storage,
                "settings were created by a newer lspotify version",
            ));
        }
...
#[cfg(test)]
mod tests {
    use lspotify_core::ThemePreference;

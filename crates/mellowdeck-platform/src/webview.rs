...
use mellowdeck_core::{AppError, ErrorKind, LocalPlayerCommand, Result};
use serde_json::json;
use wry::{
...
const PLAYER_PROTOCOL: &str = "mellowdeck";
/// `WebView2` maps `mellowdeck://localhost/` to this secure origin. Encrypted Media Extensions,
/// which the Web Playback SDK requires, are unavailable to the opaque origin of inline HTML.
const PLAYER_ORIGIN: &str = "https://mellowdeck.localhost/";
...
/// be passed through `mellowdeck_playback::parse_event` at the adapter boundary.
...
mod tests {
    ...
    fn player_navigation_is_restrictive() {
        ...
        assert!(is_allowed_player_navigation("https://mellowdeck.localhost/index.html"));
        assert!(!is_allowed_player_navigation("https://mellowdeck.localhost.example.com/"));

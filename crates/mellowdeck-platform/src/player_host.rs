...
use mellowdeck_core::{AppError, ErrorKind, LocalPlayerCommand, LocalPlayerEvent, Result};
use mellowdeck_playback::{
    BRIDGE_PROTOCOL_VERSION, BridgeEvent, Envelope, HostRecord, LocalCommand, parse_host_record,
};

const HOST_BASENAME: &str = "mellowdeck-player-host";
...
impl PlayerHostLocator {
    ...
    pub fn locate(&self) -> Result<PathBuf> {
        ...
        if let Some(path) = env::var_os("MELLOWDECK_PLAYER_HOST") {
        ...
        let candidates = if cfg!(target_os = "macos") {
            ...
            vec![
                directory.join("../lib/mellowdeck").join(name),
        ...
        candidates.into_iter().find_map(|candidate| validate_host(candidate).ok()).ok_or_else(
            || {
                AppError::new(
                    ...
                    "Mellowdeck's private player helper is missing; reinstall the application",
                )
            },
        )
    }
...
fn host_filename() -> &'static str {
    if cfg!(target_os = "windows") { "mellowdeck-player-host.exe" } else { HOST_BASENAME }
}
...
fn validate_host(path: PathBuf) -> Result<PathBuf> {
    ...
    if !metadata.is_file() {
        return Err(AppError::new(
            ErrorKind::Unavailable,
            "Mellowdeck's player helper is not a regular file",
        ));
    }
    #[cfg(unix)]
    if std::os::unix::fs::PermissionsExt::mode(&metadata.permissions()) & 0o111 == 0 {
        return Err(AppError::new(
            ErrorKind::Unavailable,
            "Mellowdeck's player helper is not executable",
        ));
    }
    Ok(path)
}
...
fn validate_hello(line: &str, nonce: &str) -> Result<()> {
    match parse_host_record(line)? {
        ...
        HostRecord::Hello(_) => Err(AppError::new(
            ErrorKind::Unavailable,
            "player helper version mismatch; reinstall Mellowdeck",
        )),
...
mod tests {
    ...
    fn package_layouts_are_private() {
        let locator = PlayerHostLocator::from_executable(
            "/Applications/Mellowdeck.app/Contents/MacOS/mellowdeck",
        );
        assert!(locator.executable.ends_with("mellowdeck"));
    }
    #[test]

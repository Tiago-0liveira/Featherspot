...
pub fn target_asset_name(os: &str, arch: &str) -> Result<String, UpdateError> {
    match (os, arch) {
        ("windows", "x86_64") => Ok("lspotify-windows-x86_64.exe".to_string()),
        ("linux", "x86_64") => Ok("lspotify-linux-x86_64".to_string()),
        ("macos", "x86_64") => Ok("lspotify-macos-x86_64".to_string()),
        ("macos", "aarch64" | "arm64") => Ok("lspotify-macos-aarch64".to_string()),
...
pub fn download_url_to_path(url: &str, destination: &Path) -> Result<(), UpdateError> {
    let client = reqwest::blocking::Client::builder()
        .user_agent(format!("lspotify-updater/{}", env!("CARGO_PKG_VERSION")))
...
mod tests {
    ...
    fn asset_selection_mapping() {
        assert_eq!(target_asset_name("windows", "x86_64").unwrap(), "lspotify-windows-x86_64.exe");
        assert_eq!(target_asset_name("linux", "x86_64").unwrap(), "lspotify-linux-x86_64");
        assert_eq!(target_asset_name("macos", "x86_64").unwrap(), "lspotify-macos-x86_64");
        assert_eq!(target_asset_name("macos", "aarch64").unwrap(), "lspotify-macos-aarch64");
        assert_eq!(target_asset_name("macos", "arm64").unwrap(), "lspotify-macos-aarch64");
    ...
    fn find_platform_and_checksum_assets() {
        let release = ReleaseInfo {
            ...
            assets: vec![
                ReleaseAsset {
                    name: "lspotify-windows-x86_64.exe".to_string(),
                    browser_download_url: "https://example.com/win".to_string(),
                    size: 100,
                },
                ReleaseAsset {
                    name: "lspotify-linux-x86_64".to_string(),
        ...
        assert_eq!(win_asset.name, "lspotify-windows-x86_64.exe");

        let linux_asset = find_asset_for_platform(&release, "linux", "x86_64").unwrap();
        assert_eq!(linux_asset.name, "lspotify-linux-x86_64");
    ...
    fn parse_checksum_formats() {
        let file_content = r"
# SHA256 checksums
ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad  lspotify-linux-x86_64
cb832e18842da70078d179c7993b6577f440274904a5f7e8a60731a4395b28e9 *lspotify-windows-x86_64.exe
SHA256 (lspotify-macos-x86_64) = 2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824
";

        let linux_hash = parse_checksum(file_content, "lspotify-linux-x86_64").unwrap();
        assert_eq!(linux_hash, "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad");

        let win_hash = parse_checksum(file_content, "lspotify-windows-x86_64.exe").unwrap();
        assert_eq!(win_hash, "cb832e18842da70078d179c7993b6577f440274904a5f7e8a60731a4395b28e9");

        let mac_hash = parse_checksum(file_content, "lspotify-macos-x86_64").unwrap();
    ...
    fn compute_and_verify_sha256() {
        let data = b"hello lspotify";
        let mut reader = &data[..];
        let hash = compute_sha256(&mut reader).unwrap();
        // SHA-256 of "hello lspotify"
        let expected = "45cf1addcd9694481a7db2be5d06c6ff5151de029fc1960f69dbd52b4f58ad3a";

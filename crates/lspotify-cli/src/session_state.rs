...
mod tests {
    ...
    fn missing_and_future_files_fall_back() {
        let root =
            std::env::temp_dir().join(format!("lspotify-session-fallback-{}", std::process::id()));
    ...
    fn round_trip_clamps_volume() {
        let root =
            std::env::temp_dir().join(format!("lspotify-session-clamp-{}", std::process::id()));
        let store = CliSessionStore::new(&root);
        store
            .save(&CliSessionState {
                schema_version: 1,
                volume: 255,
                last_track: None,
                recent_searches: Vec::new(),
    ...
    fn multiple_saves_to_same_path_succeed_and_update_content() {
        let root =
            std::env::temp_dir().join(format!("lspotify-session-multi-{}", std::process::id()));
        let store = CliSessionStore::new(&root);

...
It builds `lspotify.exe` and `lspotify-player-host.exe` together and publishes a per-user
...
stable across versions, while settings, cache, and logs remain in `%LOCALAPPDATA%\lspotify`.

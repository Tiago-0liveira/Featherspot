...
- **Header Line-by-Line Structure** (within standard 10-line detail header):
  ...
  - **Line 1 (Title)**: Type icon (`▣`, `≡`, or `◉`) paired with bold white title (`state.page.title`).
  ...
  - **Line 4 (Action Buttons)**:
    ...
    - `[ ↗ Open in Spotify ]` (conditionally rendered if `external_url` is present)
...
- **Keyboard Shortcuts**:
  - In detail routes (`Route::Album | Route::Playlist | Route::Artist`), bind:
    ...
    - `S` (Shift+S): Play detail context with shuffle
...
- **Unit Tests**:
  ...
  - `detail_context_actions_dispatch_via_mouse_and_keyboard`: Verifies mouse clicks on hit targets and keyboard keys `p`, `S`, `o` dispatch expected effects.
  - `detail_page_header_renders_buttons_and_eliminates_duplicated_info`: Renders ratatui test terminal and asserts hit targets, button strings, tracks header, and layout geometry.
